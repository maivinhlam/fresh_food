                ...
                <div class="col-md-6">
                    <div class="float-right">
                        <button type="button" href="#" title="add new user" data-type="create" class="btn btn-success show_modal_form" data-toggle="modal"
                             data-target="#modalEdit" data-title="Create" data-url="{{ route('slides.store')}}" >
                            <i class="fa fa-plus-circle"></i>
                            Create
                        </button>

                        <a href="{{route('slides.index')}}" title="refresh table for users" class="btn btn-primary" data-trigger-pjax="1" data-pjax-target="#user-grid">
                    ...
                    @foreach($slides as $slide)
                        <tr>
                            <th scope="row" class="p-1 d-flex justify-content-center align-items-center">
                                ...
                                ?>
                                {{ (($page -1) * count($slides)) + ($loop->index + 1) }}
                            </th>
                            <td>
                                <a href="#">
                                    {{ $slide->link }}
                            ...
                            <td class="" >
                                <img src="/{{ $slide->image }}" alt="{{ $slide->image }}" height="80px">
                            </td>
                            <td class="">
                                {{ $slide->creator_id }}
                            </td>
                            ...
                            <td class="project-actions p-0 text-center">
                                <div class="">
                                    ...
                                    <button type="button" class="btn btn-info btn-sm mb-1" data-type="edit" data-toggle="modal" data-target="#modalEdit"
                                        data-title="Edit"
                                        data-name="{{ $slide->link }}"
                                        data-image="{{ $slide->image }}"
                                        data-url="{{ route('slides.update', $slide->id) }}"
                                        >
                                    ...
                                    <button id="btn_delete" type="button" class="btn btn-danger btn-sm mb-1" data-toggle="modal" data-target="#modalDelete"
                                        ...
                                        data-name="{{ $slide->link }}"
                                        data-image="{{ $slide->image }}"
                                        data-url="{{ route('slides.destroy', $slide->id) }}">
            ...
            <div class="modal-body">
                <form method="POST" action="" id="formProduct" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <label for="slide_name" class="control-label">Link:</label>
                        <input type="text" class="form-control" id="slide_name" name="link">
                    </div>

                    <div class="form-group">
                        <label for="price" class="control-label">Image Link:</label>
                        <input type="text" class="form-control" id="slide_name" name="image_link">
                    </div>
                    <div class="form-group">
